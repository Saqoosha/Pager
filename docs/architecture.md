...
2. POST /request with tool name, input, project
3. Poll GET /status/:id every 2 seconds
4. On decision: output hookSpecificOutput JSON with permissionDecision
5. On timeout (120s): exit 0 (falls through to normal prompt)
6. On error: exit 0 (falls through to normal prompt)
```
